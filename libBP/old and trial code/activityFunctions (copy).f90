...
! Using L-1(1/(q+ps) = (1/p)*exp(-1*(q/p)*t) and partial fractions
    Do i=1,decayChain%isotopes
      !nEnd = CalcIsotopeChainCalcWorks(t,i,W,L,N,E,B)
    ...
    !Do i=1,decayChain%isotopes
      !nEnd = CalcIsotopeChainCalcWorks(t,i,W,L,N,B)
      !decayChain%amountEnd(i) = nEnd
    !End Do
    print *,""
    print *,""
  ...
  ! Loop through pfrac
      Do i=k,m
        ...
        Do j=k,m
          ...
          End If
        End Do
        print *,(1.0D0/r)*exp(-1.0D0*L(i)*t)*multP*mult
        nChange = nChange + (1.0D0/r)*exp(-1.0D0*L(i)*t)*multP*mult
      End Do
...
! Loop through all combinations (order important)
  ...
  Function CalcIsotopeChainCalcWorks(t,z,W,L,N,B) Result (nEnd)
    Implicit None ! Force declaration of all variables
  ! Vars Private
    Integer(kind=StandardInteger) :: i, ii, termKey, j, jj, z
    Integer(kind=StandardInteger) :: termI, pfracI, bracI, nKey, lKey, lMin, startL
    Real(kind=DoubleReal) :: q, p, r, s, t, nEnd, pfMultiplier, termMultiplier, partVal
    Real(kind=DoubleReal), Dimension(1:100) :: W ! Production Rate
    Real(kind=DoubleReal), Dimension(1:100) :: L ! Lambda
    Real(kind=DoubleReal), Dimension(1:100) :: N ! Starting number of atoms
    Real(kind=DoubleReal), Dimension(1:100) :: B ! Exp
...
! Loop through pfrac term
        Do pfracI=1,termI
          ...
          End If
          r = partVal
          p = 1.0D0 * pfMultiplier * r
          q = L(lKey)*p
          s = (1.0D0/p)*exp(-1.0D0*(q/p)*t)*termMultiplier*N(nKey)
          nEnd = nEnd + s
          If(z.eq.3)Then
            print *,termI,pfracI
            print *,"  p    ",p
...
! Remaining terms
      ...
      End Do
    End If
  End Function CalcIsotopeChainCalcWorks
