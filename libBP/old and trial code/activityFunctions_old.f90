...
! Loop and alter matching decay constants slightly
  ...
  ! Vars Private
    ...
    Integer(kind=StandardInteger) :: termI, pfracI, bracI, nKey, lKey, wKey, lMin, startL
...
! Loop through pfrac term
        Do pfracI=1,termI
          lKey = z-termI+pfracI
          print *,"   pfrac ",pfracI," of ",termI,"  lKey ",lKey
          ...
          If(termI.ge.2)Then
            Do i=lMin,z
              If(i.ne.lKey)Then
                partVal = partVal * (L(lKey)-L(i))
                print "(A2,I1,A4,I1,A1,E16.8)","L(",lKey,")-L(",i,")",(L(lKey)-L(i))
              End If
            End Do
          ...
          p = 1.0D0 * pfMultiplier * partVal
          q = L(lKey)*p
          r = (1.0D0/p)*exp(-1.0D0*(q/p)*t)*termMultiplier*W(wKey)
          print *,w(wKey),r
          nEnd = nEnd + r
...
! Loop through all combinations (order important)
  ...
  Function CalcIsotopeChainCalcWorks(t,z,W,L,N,E,B) Result (nEnd)
    Implicit None ! Force declaration of all variables
  ! Vars Private
    Integer(kind=StandardInteger) :: i, ii, termKey, j, jj, z
