...
! Public derived types
  ...
  Type :: coordsType
    ...
    Real(kind=DoubleReal), Dimension(1:p_cMax,1:3) :: coords       ! coords
...
! Coord details
    ...
    Real(kind=DoubleReal), Dimension(1:p_cMax,1:3) :: fracCoords   ! Fractional coords
    Real(kind=DoubleReal), Dimension(1:p_cMax,1:3) :: forces   ! Forces
! Atom details
...
! Fractional coords
                coords(cKeyE)%fracCoords(m,1) = (coordsUnit(cKey)%unitCoords(n,1)+1.0D0*(i-1))/(1.0D0*coordsUnit(cKey)%xCopy)
                coords(cKeyE)%fracCoords(m,2) = (coordsUnit(cKey)%unitCoords(n,2)+1.0D0*(j-1))/(1.0D0*coordsUnit(cKey)%yCopy)
                coords(cKeyE)%fracCoords(m,3) = (coordsUnit(cKey)%unitCoords(n,3)+1.0D0*(k-1))/(1.0D0*coordsUnit(cKey)%zCopy)
! Real coords
                ...
                coords(cKeyE)%coords(m,1) = xCoords(1)
                coords(cKeyE)%coords(m,2) = xCoords(2)
                coords(cKeyE)%coords(m,3) = xCoords(3)
! Forces
...
! Store from coords
    ...
    nl(cKey)%fracCoords = coords(cKey)%fracCoords
    nl(cKey)%forces = coords(cKey)%forces
!----------------------------------------------
...
! Update atom seperation
      xD = aLat*(nl(cKey)%fracCoords(nl(cKey)%atomA_ID(n),1)-nl(cKey)%fracCoords(nl(cKey)%atomB_ID(n),1)+&
             nl(cKey)%subCell(n,1)) ! xA - yA
      yD = aLat*(nl(cKey)%fracCoords(nl(cKey)%atomA_ID(n),2)-nl(cKey)%fracCoords(nl(cKey)%atomB_ID(n),2)+&
             nl(cKey)%subCell(n,2)) ! xB - yB
      zD = aLat*(nl(cKey)%fracCoords(nl(cKey)%atomA_ID(n),3)-nl(cKey)%fracCoords(nl(cKey)%atomB_ID(n),3)+&
      ...
      If(n.lt.10)Then
        print *,nl(cKey)%fracCoords(nl(cKey)%atomA_ID(n),1),nl(cKey)%fracCoords(nl(cKey)%atomB_ID(n),1)
      End If
